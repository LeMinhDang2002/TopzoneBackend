<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="lastname" class="block mt-1 w-full" type="text" name="phone" :value="old('phone')" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('phone')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="type_user" :value="__('Type User')" />
            <x-text-input id="type_user" class="block mt-1 w-full" type="text" name="type_user" :value="old('type_user')" required autofocus autocomplete="name" />
